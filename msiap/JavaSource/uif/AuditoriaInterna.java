 ...
 * </p>
 */
public class AuditoriaInterna extends AbstractPageBean {
	static MyLogger logger= new MyLogger(AuditoriaInterna.class.getName());
	...
	public void init() {
		...
		try {
			...
			logger.error("auditoria Initialization Failure", e);
		...
		// tbMovimientos.setTitle("Detalles de la auditoría. Solicitud Nro. " +
		// String.valueOf(getSessionBean1().getSolicitudExterna().getCODG_CERTI_EXTERNO()));
	}
	...
	public String btnSeguimiento_action() {
		/*
		 * String auditoria = null; com.sun.data.provider.RowKey rowkey =
		 * tableRowGroup2.getRowKey(); if
		 * (getSessionBean1().getEstadoSolicitudes().getValue("DESC_FEC_USU_MODI",rowkey) !=
		 * null){ auditoria =
		 * String.valueOf(getSessionBean1().getEstadoSolicitudes().getValue("DESC_FEC_USU_MODI",rowkey));
		 * auditoria = auditoria.replaceAll(";","\n"); }
		 * taSeguimiento.setRendered(true); taSeguimiento.setValue(auditoria);
		 */
		return null;
	}
